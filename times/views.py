...
def wiki(request,wid):
	...
	wtypes = WikiType.objects.all()
	return render(request,'view-wiki.html',{'this_wiki':this_wiki,'paginas':collection,'wtypes':wtypes})
...
def homepage(request):
	...
	return render(request,'homepage.html',{'articles':articles,'dpaginas':dpaginas,'npages':range(npages),'on_reading':on_reading})
...
def watchshow(request):
	...
	newW = ShowWatch.objects.create(sseason = this_season,start_d=inicio,finish_d=fin)
...
def booklists(request):
	listas = BookList.objects.all().order_by('-id')
...
def statistics(request):
	...
	return render(request, 'stats.html', { "data_points" : data_points, "data_points2":data_points2 })
